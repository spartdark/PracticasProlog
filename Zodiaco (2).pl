...
signop(Nombre,s):-write('Que signo le interesa?'),read(Signoab),nl,write('Que edad le interesa(Menor,Mayor o indistinto)?'),read(Edad),write('¿Que sexo busca?/hombre,mujer,indistinto'),read(Sexab),edads(Nombre,Signoab,Edad, Sexab),fail.

signop(Nombre,n):-write('Que edad le interesa(Menor,Mayor o indistinto)?'),read(Edad),edad(Nombre,Edad),fail.


edads(Nombre,Signoab,menor,Sexab):-persona(Nombre,A,B,_,_),persona(X,Y,Z,_,_),Z=<B, sexop(Nombre,Signoab,A,B,X,Y,Z,Sexab).
edads(Nombre,Signoab,mayor, Sexab):-persona(Nombre,A,B,_,_),persona(X,Y,Z,_,_),Z>=B,sexop(Nombre,Signoab,A,B,X,Y,Z,Sexab).
edads(Nombre,Signoab,indistinto, Sexab):-persona(Nombre,A,B,_,_),persona(X,Y,Z,_,_),sexop(Nombre,Signoab,A,B,X,Y,Z,Sexab).
